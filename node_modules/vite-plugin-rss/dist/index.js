...
__export(src_exports, {
  isImage: () => isImage,
  isJsXmlElement: () => isJsXmlElement,
  isSkipDays: () => isSkipDays,
  isSkipHours: () => isSkipHours,
      ...
      server.middlewares.use((req, res, next) => {
        if (typeof req.url === "string" && new RegExp(`${fileName}$`).test(req.url)) {
          if (opts.mode === "meta") {
            ...
            items = moduleInfo.filter((module2) => !!(module2 == null ? void 0 : module2.meta.rssItem)).map((module2) => module2.meta.rssItem);
          }
          const renderedXML = createRssFeed(opts.channel, items != null ? items : [], fileName);
          const fileContent = Buffer.from(renderedXML, "utf8");
          const readStream = new import_stream.default.PassThrough();
...
function createRssFeed(channel, items, fileName) {
  ...
  return import_xml_js.default.js2xml(jsXml, { compact: true, spaces: 2 });
}
